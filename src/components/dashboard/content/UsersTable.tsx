...
function convertToCSV(users: any[]) {
  const headers = ["#", "Name", "Mobile", "Email/Username"];
...
export function UsersTable() {
  const { data, loading, error } = useFetch<any[]>("db1/users");
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="bg-violet-50 dark:bg-violet-900/40 border-b-2 border-dashed border-violet-300 dark:border-violet-700">
              <tr>
                ...
                <th className="px-4 py-2 font-bold text-violet-700 dark:text-violet-300">Mobile</th>
                <th className="px-4 py-2 font-bold text-fuchsia-700 dark:text-fuchsia-300">Email / Username</th>
              </tr>
            </thead>
            <tbody>
              {filteredUsers.map((user, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 font-mono">{user.mobile}</td>
                  <td className="px-4 py-2 font-mono text-muted-foreground">{user.user}</td>
                </tr>
              ))}
            </tbody>
          </table>
